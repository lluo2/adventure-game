 ...
 * expansion capabilities as well as easy accessibility to succeeding vertices/weights.
 */
public class AdjMatGraph<T> implements Graph<T>, Iterable<T> {
	...
	private class VerticesIterator implements Iterator<T> {
		...
		public boolean hasNext() { return cursor < n; }
	 ...
	 */
	public Iterator<T> iterator() { return new VerticesIterator(); }
	 ...
	 */
	public T[] getVertices() { return vertices; }
	...
	public boolean isUndirected() {
		for (int i = 1; i < n(); i++) {
			...
			for (int j = 0; j < i; j++) {
				if (arcs[i][j] != arcs[j][i]) return false;
			}
		}
		return true;
	}	
	...
	public void addVertex (T vertex) {
		...
		if (n == vertices.length) expandCapacity();
	...
	public void removeVertex (T vertex) {
		...
		if (index != NOT_FOUND) removeVertex(index);
	}
